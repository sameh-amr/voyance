...
export const encryptObject = (obj: Record<string, any>): Record<string, any> => {
  ...
  Object.keys(obj).forEach((key) => {
    const encryptedValue = CryptoJS.AES.encrypt(JSON.stringify(obj[key]), process.env.REACT_APP_ENCRYPTIONKEY??"").toString();
...
export const decryptObject = (encryptedObj: Record<string, any>): Record<string, any> => {
  const decryptedObj: Record<string, any> = {};

  Object.keys(encryptedObj).forEach((key) => {
    const decryptedValue = CryptoJS.AES.decrypt(encryptedObj[key], process.env.REACT_APP_ENCRYPTIONKEY??"").toString(CryptoJS.enc.Utf8);
